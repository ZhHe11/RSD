...
def PCA_plot_traj(All_Repr_obs_list, All_Goal_obs_list, path, path_len=100, is_PCA=False, is_goal=True, ax=None, colors=None):
...
## Main interation:
@torch.no_grad()
def eval_cover_rate(env, agent_traj_encoder, agent_policy, dim_option, device, ax=None, max_path_length=300, Psi=_Psi, option_type=None, eval_num=100):
...
## For viz Repr in Maze:
@torch.no_grad()
def PlotMazeTrajDist(env, SZN, input_token, agent_traj_encoder, qf1, qf2, alpha, policy, device, Psi, dim_option=2, max_path_length=300, path='./', eval_num=100):    
    obs0 = env.reset()
    ...
    ax[0,0], All_Repr_obs_list, All_Goal_obs_list, All_trajs_list, FinallDistanceList, ArriveList = eval_cover_rate(env, agent_traj_encoder, policy, dim_option, device, ax=ax[0,0], max_path_length=max_path_length, Psi=Psi, eval_num=eval_num)
    # calculate metrics
...
## For viz Repr in Maze:
@torch.no_grad()
def PlotMazeTrajWindowDist(env, window, agent_traj_encoder, qf1, qf2, alpha, policy, device, Psi, dim_option=2, max_path_length=300, path='./', option_type=None, eval_num=100): 
    obs0 = env.reset()
    s0 = torch.tensor(obs0).to(device).float()
    fig, ax = plt.subplots(2,2)
    fig.subplots_adjust(wspace=0.4, hspace=0.4) 
    ...
    ax[0,0], All_Repr_obs_list, All_Goal_obs_list, All_trajs_list, FinallDistanceList, ArriveList, All_Cover_list = eval_cover_rate(env, agent_traj_encoder, policy, dim_option, device, ax=ax[0,0], max_path_length=max_path_length, Psi=Psi, option_type=option_type, eval_num=eval_num)
    # calculate metrics
    FD = np.array(FinallDistanceList).mean()
    AR = np.array(ArriveList).mean()
    print("FD:", FD, '\n', "AR:", AR)
...
def PlotMazeTraj(env, agent_traj_encoder, policy, device, Psi, dim_option=2, max_path_length=300, path='./', option_type=None, eval_num=100): 
    ...
    ax[0], All_Repr_obs_list, All_Goal_obs_list, All_trajs_list, FinallDistanceList, ArriveList, All_Cover_list = eval_cover_rate(env, agent_traj_encoder, policy, dim_option, device, ax=ax[0], max_path_length=max_path_length, Psi=Psi, option_type=option_type, eval_num=eval_num)
    # calculate metrics
    FD = np.array(FinallDistanceList).mean()
    AR = np.array(ArriveList).mean()
    print("FD:", FD, '\n', "AR:", AR)
